...
-   ***Figure 1*** appears to be positive. The relationship looks reasonably linear because the points generally follow an upward straight-line pattern.
:::
...
-   No, it is not reasonable to conclude that social policies that increase white poverty will cause an increase in Trump support. Just because poverty and Trump support move together does not mean poverty causes that support. **Correlation does not necessarily cause causation**
:::
...
-   Given the linear model, `poverty_mod`, we can expect the support for Trump to be **43.16%** when the white poverty is **6%**
:::
...
-   No, because 85% poverty is way outside your actual data range and gives you a totally impossible result of over 200% Trump support.
:::
...
```{r}
ggplot(data = trump, aes(x = poverty, y = trump_support)) +
  geom_point(color = "black") +
  geom_smooth(method = "lm", se = FALSE, color = "darkred", linewidth = 1) +
  labs(
    title = "Relationship Between Non-White Population % and Trump Support",
    x = "Non-White Population Percentage (%)",
    y = "Trump Support Percentage (%)"
  ) +
  theme_bw()
```
:::
...
-   Should expect the correlation coefficient to be negative

```{r}
# Type your code and comments inside the code chunk
r_value <- cor(trump$non_white, trump$trump_support, use = "complete.obs")

cat("Correlation coefficient (r):", round(r_value, 4), "\n")

cor.test(trump$non_white, trump$trump_support)
...
-   With a 0% `non-white population`, the expected baseline Trump support is 58.3%.

-   For every 1% increase in the `non-white population`, Trump support decreases by an average of 0.269 percentage points.
:::
...
predicted_support <- intercept + (slope * non_white_pct)
cat("Predicted Trump Support:", predicted_support, "%\n")

new_region <- data.frame(non_white = 70)
predict(nw_mod, newdata = new_region)

```

-   The expected level of Trump support for a region with a 70% non-white population is **39.47%**.
:::
...
-   Based on the model statistics, `poverty_mod` is the better explanatory variable for Trump support.
:::
...
**A.** The predicted trump support in a region where **61%** of people identify non-white would be **41.9%**
...
**B.** Her prediction is an overestimate by 8.91 percentage points. The model predicted 41.91%, but the actual support in that region was only 33%.

**C.** The `residual` for this prediction is -8.912
:::
